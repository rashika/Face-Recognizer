...
imagecount=0;
 ...
 fclose(fileID3);
 scale = 100;
 trainlabel=zeros(train_count,1);
 testlabel=zeros(test_count,1);
 ...
 count=1;
 while ischar(trlbl)
     trainlabel(count,1) = str2num(trlbl);
     count=count+1;
     trlbl = fgetl(fid1);
 end
 ...
 telbl = fgetl(fid2);
 while ischar(telbl)
     testlabel(count,1) = str2num(telbl);
     count=count+1;
     telbl = fgetl(fid2);
 end
...
accuracy=[];
imgweight = zeros(size(eigenfaces,2),test_count);
...
for iter=1:size(eigen_arr,2),
    ...
    for kcount=1:size(k_arr,2),
        ...
        for i=1:size(testimg,2),
            ...
            imgweight(:,i) = eigenfaces'*test_temp(:,i);
        end
     end
end
